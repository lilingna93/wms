...
$warData = $projectSummarydata["war"];//此为包含退货数据的货品
...
function getMonthData($frequencyWarData, $stime, $etime)
{
    ...
    foreach ($ordergoodsData as $ordergoodsDatum) {
        ...
        if(2==$isExternal){
            $sheetName="市场部项目销售数据表数据(外部项目组)";
        }elseif(1==$isExternal){
            $sheetName="市场部项目销售数据表数据(内部项目组)";
        }else{
        ...
        $warData[$sheetName][$warName][$spuType]['frequency'] = $frequencyWarData[$warCode][$ordergoodsDatum['spu_type']];
    }
    $data = array(
        "war" => $warData,
    );
