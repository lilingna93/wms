...
class ExcelService
{
    ...
    public function uploadByShell($filePath)
    {
        ...
        for ($i = 0; $i < $sheetCount; $i++) {
            ...
            $letters = array();
            $rows = 100;
            for ($letter = 0; $letter < $rows; $letter++) {
                $letters[] = $this->getLettersCell($letter);
            }
            if (!in_array($allColumn, $letters)) venus_throw_exception(25);
    ...
    function exportExcelByHeader($data, $fileHeader, $typeName)
    {
        ...
        if (file_exists($filePath)) {
            ...
            foreach ($data as $sheetName => $list) {
                ...
                $lettersLast = $this->getLettersCell($lettersCount - 1);
                $endStyleLine = $line - 1;
//                $sheet->getColumnDimension("A1:$lettersLast$endStyleLine")->setAutoSize(true);
                $sheet->getStyle("A1:$lettersLast$endStyleLine")->applyFromArray(
                    array(
                        'borders' => array(
                            'allborders' => array(
...
//            exit();
    ...
    public function outPut($dir, $fileName, $saveFile)
    {
        ...
        \Think\Log::write('开始日志记录----'.$file_path,'zk0701-a');
