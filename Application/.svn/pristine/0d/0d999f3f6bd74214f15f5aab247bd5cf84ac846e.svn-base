    ...
    foreach ($spuArr as $spuType => $spuData) {
        foreach ($spuData as $spuName) {
            if (array_key_exists($spuName, $prevData[$spuType])) {
                $data[$spuType][$spuName]['prev'] = $prevData[$spuType][$spuName]['prev'];
            }
            if (array_key_exists($spuName, $currentData[$spuType])) {
                if (array_key_exists("rec", $currentData[$spuType][$spuName])) {
                    $data[$spuType][$spuName]['rec'] = $currentData[$spuType][$spuName]['rec'];
                }
                if (array_key_exists("4", $currentData[$spuType][$spuName])) {
                    $data[$spuType][$spuName]['4'] = $currentData[$spuType][$spuName]['4'];
                }
                if (array_key_exists("5", $currentData[$spuType][$spuName])) {
                    $data[$spuType][$spuName]['5'] = $currentData[$spuType][$spuName]['5'];
                }
                if (array_key_exists("6", $currentData[$spuType][$spuName])) {
                    $data[$spuType][$spuName]['6'] = $currentData[$spuType][$spuName]['6'];
                }
                if (array_key_exists("7", $currentData[$spuType][$spuName])) {
                    $data[$spuType][$spuName]['7'] = $currentData[$spuType][$spuName]['7'];
                }
            }
        }
    }
...
//        }else{
        $data[$spuType][$spuName]['rec'] = floatval(bcadd($data[$spuType][$spuName]['rec'], $bprice, 8));
//        }
        if (!in_array($spuName, $spuArr[$spuType])) {
            $spuArr[$spuType][] = $spuName;
        }
...
//        }else{
        $data[$spuType][$spuName][$invTypeData[$invCode]] = floatval(bcadd($data[$spuType][$spuName][$invTypeData[$invCode]], $bprice, 8));
//        }
        if (!in_array($spuName, $spuArr[$spuType])) {
            $spuArr[$spuType][] = $spuName;
        }
...
//        echo $typeCell.PHP_EOL;
        ...
        foreach ($currentDatum as $spuName => $current) {
            ...
            foreach ($current as $type => $money) {
                if ($type == "prev") {
                    $prevCell = "D" . $line;
                    $excelData["库存表"][$prevCell] = $money;
                }
                if ($type == "rec") {
                    $prevCell = "E" . $line;
                    $excelData["库存表"][$prevCell] = $money;
            ...
            $invTotalCell = "J" . $line;
            $excelData["库存表"][$invTotalCell] = "=F$line+G$line+H$line+I$line";
            $goodsTotalCell = "K" . $line;
            $excelData["库存表"][$goodsTotalCell] = "=D$line+E$line-J$line";
            $spuTypeLine++;
            $line++;
        }
        $endLine = $line;
...
//        echo "A" . $startLine . ":" . "A" . $endLine . PHP_EOL;
        ...
        $recTotalCell = "D" . $endLine;
        $excelData["库存表"][$recTotalCell] = "=SUM(D$startLine:D$stopLine)";
        $recCurTotalCell = "E" . $endLine;
        $excelData["库存表"][$recCurTotalCell] = "=SUM(E$startLine:E$stopLine)";
        $sellTotalCell = "F" . $endLine;
        $excelData["库存表"][$sellTotalCell] = "=SUM(F$startLine:F$stopLine)";
        $applyTotalCell = "G" . $endLine;
        $excelData["库存表"][$applyTotalCell] = "=SUM(G$startLine:G$stopLine)";
        $lessTotalCell = "H" . $endLine;
        $excelData["库存表"][$lessTotalCell] = "=SUM(H$startLine:H$stopLine)";
        $returnTotalCell = "I" . $endLine;
        $excelData["库存表"][$returnTotalCell] = "=SUM(I$startLine:I$stopLine)";
        $invTotalCell = "J" . $endLine;
        $excelData["库存表"][$invTotalCell] = "=F$endLine+G$endLine+H$endLine+I$endLine";
        $goodsTotalCell = "K" . $endLine;
        $excelData["库存表"][$goodsTotalCell] = "=D$endLine+E$endLine-J$endLine";
        $lineArr[] = $endLine;
    }
    $totleLine = $line;
    ...
    foreach ($lineArr as $key => $lineData) {
        if ($key != count($lineArr) - 1) {
            $prevTotalStr = $prevTotalStr . "D" . $lineData . "+";
            $recTotalStr = $recTotalStr . "E" . $lineData . "+";
            $sellTotalStr = $sellTotalStr . "F" . $lineData . "+";
            $applyTotalStr = $applyTotalStr . "G" . $lineData . "+";
            $lessTotalStr = $lessTotalStr . "H" . $lineData . "+";
            $returnTotalStr = $returnTotalStr . "I" . $lineData . "+";
        } else {
            $prevTotalStr = $prevTotalStr . "D" . $lineData;
            $recTotalStr = $recTotalStr . "E" . $lineData;
            $sellTotalStr = $sellTotalStr . "F" . $lineData;
    ...
    $prevCurTotalCell = "D" . $totleLine;
    $excelData["库存表"][$prevCurTotalCell] = $prevTotalStr;
    $recCurTotalCell = "E" . $totleLine;
    $excelData["库存表"][$recCurTotalCell] = $recTotalStr;
    $sellTotalCell = "F" . $totleLine;
    $excelData["库存表"][$sellTotalCell] = $sellTotalStr;
    $applyTotalCell = "G" . $totleLine;
    $excelData["库存表"][$applyTotalCell] = $applyTotalStr;
    $lessTotalCell = "H" . $totleLine;
    $excelData["库存表"][$lessTotalCell] = $lessTotalStr;
    $returnTotalCell = "I" . $totleLine;
    $excelData["库存表"][$returnTotalCell] = $returnTotalStr;
    $invTotalCell = "J" . $totleLine;
    $excelData["库存表"][$invTotalCell] = "=F$totleLine+G$totleLine+H$totleLine+I$totleLine";
    $goodsTotalCell = "K" . $totleLine;
    $excelData["库存表"][$goodsTotalCell] = "=D$totleLine+E$totleLine-J$totleLine";
//    exit();
    return $excelData;
