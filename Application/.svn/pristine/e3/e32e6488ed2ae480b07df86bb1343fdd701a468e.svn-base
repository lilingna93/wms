...
$excelWriter->save("goodsskudata080902 .online.xlsx");
exit;
...
function exportGoodbatchData($skucode, $excelSheet)
{
    ...
    $line++;
    $list = M("goodsbatch")->where(array("sku_code" => $skucode,"gb_code"=>array(array("GT","GB40802000000000"),array("LT","GB40803000000000"),'AND')))->fetchSql(false)->select();
...
function exportGoodstoredData($skucode, $excelSheet)
{
    ...
    $line++;
    $list = M("goodstored")->where(array("sku_code" => $skucode,"gs_code"=>array(array("GT","GW40802000000000"),array("LT","GW40803000000000"),'AND')))->fetchSql(false)->select();
    foreach ($list as $data) {
...
function exportIgoodsData($skucode, $excelSheet)
{
    ...
    $line++;
    $list = M("igoods")->where(array("sku_code" => $skucode,"igo_code"=>array(array("GT","GO40802000000000"),array("LT","GO40803000000000"),'AND')))->select();//,"igo_code"=>array("GT","GO31229000000000")
    ...
    foreach ($list as $data) {
        ...
        $excelSheet->setCellValue("H{$line}", $data['inv_code']);
...
function exportIgoodsentData($skucode, $excelSheet)
{
    ...
    $line++;
    $list = M("igoodsent")->where(array("sku_code" => $skucode,"igo_code"=>array(array("GT","GO40802000000000"),array("LT","GO40803000000000"),'AND')))->select();//,"igo_code"=>array("GT","GO31229000000000")
    $count = 0;
    foreach ($list as $data) {
...
function exportOrderGoodsData($skucode, $excelSheet)
{
    ...
    $line++;
    $list = M("ordergoods")->where(array("sku_code" => $skucode, "goods_code" => array(array("GT","G40802000000000"),array("LT","G40803000000000"),'AND')))->select();
    foreach ($list as $data) {
