...
$time = venus_script_begin("鲜鱼水菜订货数据表（周、月报）");

$stime = "2019-04-23 00:00:00";
$etime = "2019-04-24 00:00:00";
...
//          $cpgpr = bcdiv($goodsDatum['totalCprofit'], $grossProfit, 4);//客户利润占毛利比
            $OrderExport["订单汇总"][] = array($warname, $goodsDatum['sku_code'], $goodsDatum['spu_name'], $goodsDatum['sku_norm'], $goodsDatum['sku_unit'], $goodsDatum['sku_count'], $skSprice, $weekTotalSprice);
        }
//            }
//        }
...
$fileName = ExcelService::getInstance()->exportExcel($OrderExport, '', "002");
if (!empty($fileName)) {
    $time = venus_current_datetime();
    $newFname = "ffv_orderSummary.xlsx";
