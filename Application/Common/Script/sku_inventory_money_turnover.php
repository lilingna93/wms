...
$time = venus_script_begin("SKU库存金额周转率");
...
$skuData = array();
$fname = "SKU库存金额周转率";
$header = array("货号", "名称", "单位", "总出库金额", "平均库存金额", "库存金额周转率");
...
foreach ($spuList as $index => $spuItem) {
    ...
    if (in_array($skCode, $orderGoodsSkuCodeList)) {
        foreach ($orderGoodsList as $ordergoods => $ordergoodsItem) {
            ...
            $skuSprice = bcmul($ordergoodsItem['goods_count'], $ordergoodsItem['spu_sprice'], 4);
            $averageInventoryMoney = bcdiv($skuSprice,$days,3);
            $skuInventoryTurnover = bcdiv($skuSprice,$averageInventoryMoney,3);//库存金额周转率
            $skuData[] = array(
                $ordergoodsItem['sku_code'], $ordergoodsItem['spu_name'], $ordergoodsItem['sku_unit'], $skuSprice,
                $averageInventoryMoney, $skuInventoryTurnover,
            );
        }
    } else {
        $skuData[] = array(
...
$excelData = array();
$skuInventoryMoneyTurnover = array();
...
foreach ($excelData as $index => $val) {
    $skuInventoryMoneyTurnover[$fname][] = array(
        "0" => $val['skCode'],
        "1" => $val['spName'],
        "2" => $val['skUnit'],
        "3" => $val['skuSprice'],
        "4" => $val['averageInventoryMoney'],
        "5" => empty($val['skuInventoryTurnover']) ? 0 : $val['skuInventoryTurnover'],
    );
}
$fileName = ExcelService::getInstance()->exportExcel($skuInventoryMoneyTurnover, $header, "001");
...
if ($fileName) {
    ...
    $attachment = array(
        "SKU库存金额周转率.xlsx" => C("FILE_SAVE_PATH") . "001/" . $fileName,
    );
