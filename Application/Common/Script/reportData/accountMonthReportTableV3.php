...
function getMonthDataPMPT($stime, $etime, $skuCodeExtra)
{
    ...
    foreach ($ordergoodsData as $ordergoodsDatum) {
        ...
        $isExternal = M("$dbName.warehouse")->where(array("war_code" => $warCode))->getField("war_is_external");
        if (2 == $isExternal) {
            $sheetName = "财务部月度财务报表(外部项目组)";
        } elseif (1 == $isExternal) {
            $sheetName = "财务部月度财务报表(内部项目组)";
        } else {
        ...
        $warData[$sheetName][$warName][$spuType]['money'] = floatval(bcadd($warData[$sheetName][$warName][$spuType]['money'], $sprice, 8));
        $warData[$sheetName][$warName][$spuType]['bprice'] = floatval(bcadd($warData[$sheetName][$warName][$spuType]['bprice'], $bprice, 8));
        $warData[$sheetName][$warName][$spuType]['count'] = floatval(bcadd($warData[$sheetName][$warName][$spuType]['count'], $skuCount, 8));
    }
 ...
 * @return array
 */
function get_war_excel_data_pmpt($warDataArr, $stime, $etime)
{
    $excelData = array();
    foreach ($warDataArr as $sheetName=>$warData) {
        ...
        foreach ($warData as $warName => $warDatum) {
            ...
            foreach ($warDatum as $spuType => $warItem) {
                ...
                $excelData[$sheetName][$pppriceCell] = "=($ppriceCell*100)/$spriceCell";
            }
            ...
            $excelData[$sheetName][$totalPppriceCell] = "=($totalPpriceCell*100)/$totalSpriceCell";
            $line++;
        }
