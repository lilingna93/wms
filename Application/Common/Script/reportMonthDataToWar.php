...
$returnDataArray = $supTypeData["return"];
echo json_encode($returnDataArray) . PHP_EOL;
exit();
//$spuArr = array();
...
function getOwnMonthData($stime, $etime)
{
    ...
    foreach ($ordergoodsData as $ordergoodsDatum) {
        ...
        $spuTypeData[$warName][$spuName]['money'] = floatval(bcadd($spuTypeData[$warName][$spuName]['money'], $sprice, 8));
        $spuTypeData[$warName][$spuName]['bprice'] = floatval(bcadd($spuTypeData[$warName][$spuName]['bprice'], $bprice, 8));
        $spuTypeData[$warName][$spuName]['count'] = floatval(bcadd($spuTypeData[$warName][$spuName]['count'], $skuCount, 8));
        ...
        if ($orderTimeArr[$orderCode] > '2019-03-15 00:00:00') {
            ...
            $spuTypeData[$warName][$spuName]['pprice'] = floatval(bcadd($spuTypeData[$warName][$spuName]['pprice'], bcmul($sprice, 0.1, 4), 8));
        } else {
            ...
            $spuTypeData[$warName][$spuName]['pprice'] = floatval(bcadd($spuTypeData[$warName][$spuName]['pprice'], $pprice, 8));
        }
    }
    $condition = array();
    ...
    if (!empty($returnTaskData)) {
        ...
        $returnData = M("ordergoodsreturn")->alias("ogr")->field("*,ogr.spu_code,ogr.spu_bprice,ogr.supplier_code")
            ->join("left join wms_spu spu on spu.spu_code=ogr.spu_code")
            ->where(array(
                    "apply_time" => array(array('EGT', $stime), array('ELT', $etime), 'AND',),
                    "ogr.supplier_code" => "SU00000000000001",
                    "ogr_status" => 2)
            )
            ->limit(0, 1000000)->select();
    }
    if (!empty($returnData)) {
        foreach ($returnData as $returnDatum) {
            ...
            $spuTypeData[$warName][$spuName]['money'] = floatval(bcsub($spuTypeData[$warName][$spuName]['money'], $sprice, 8));
            $spuTypeData[$warName][$spuName]['bprice'] = floatval(bcsub($spuTypeData[$warName][$spuName]['bprice'], $bprice, 8));
            $spuTypeData[$warName][$spuName]['count'] = floatval(bcsub($spuTypeData[$warName][$spuName]['count'], $returnCount, 8));
            if ($orderTimeArr[$orderCode] > '2019-03-15 00:00:00') {
                ...
                $spuTypeData[$warName][$spuName]['pprice'] = floatval(bcsub($spuTypeData[$warName][$spuName]['pprice'], bcmul($sprice, 0.1, 4), 8));
            } else {
                ...
                $spuTypeData[$warName][$spuName]['pprice'] = floatval(bcsub($spuTypeData[$warName][$spuName]['pprice'], $pprice, 8));
            }
        }
    }
