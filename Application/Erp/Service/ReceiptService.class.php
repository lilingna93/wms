...
//        $this->worCode = $workerData["wor_code"];// = 'WO40428134034434';//'仓库';
    ...
    public function search($param)
    {
        ...
        $skuDataList = $skuModel->queryListByCondition($clause, 0, 1000);
        ...
        $totalCount = $recModel->queryCountByCondition($clauseRec);
        if (!empty($data['pageSize'])) {
            $pageLimit = pageLimit($totalCount, $pageCurrent, $data['pageSize']);
        } else {
            $pageLimit = pageLimit($totalCount, $pageCurrent);
        }
        $recDataList = $recModel->queryListByCondition($clauseRec, $pageLimit['page'], $pageLimit['pSize']);
        $returnData = array(
            ...
            foreach ($goodsCodeArr as $goodsCode => $goodsCount) {
                ...
                foreach ($recDataList as $recData) {
                    ...
                    $orderLogsDataStr .= "  " . $skuNameArr[$goodsCode]['name'] . ":"
                        ...
                        "采购价:" . $returnGoodsData["bprice"] . PHP_EOL;
